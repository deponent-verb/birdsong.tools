test_that("add_transitions works",{
  ...
  #test1 ----
  unit_table = tibble::tibble(start = c(0.37, 0.6, 0.2, 1.8), end = c(0.45, 0.7, 0.4, 2), pos = c(1,2,1,2),
                              sound.files = c("JS001.wav","JS001.wav","JS002.wav","JS002.wav")
                              ,note_label = c("A","B","C","D"))
  output = add_transitions(unit_table) 
  ans = tibble::tibble(start = c(0.37, 0.6, 0.2, 1.8), end = c(0.45, 0.7, 0.4, 2), pos = c(1,2,1,2),
                       sound.files = c("JS001.wav","JS001.wav","JS002.wav","JS002.wav")
                       ,note_label = c("A","B","C","D"),
  ...
  #test2 ----
  unit_table = tibble::tibble(start = seq(6), end = seq(6)+0.5, pos = c(1,2,3,3,1,2),
